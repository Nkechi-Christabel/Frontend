...
const handleClick = (e) => {
  ...
  return (
    <div className="App">
       <Header handleChange={handleChange} inputValue={searchInput} handleGenderSelect={handleGenderSelect} genderValue={genderFilter} handlePaymentSelect={handlePaymentSelect} paymentValue={paymentFilter} />
       ...
       {
         NumberOfPagesArr.map( number =>  <button key={number} className={number === currentPage ? 'active' : ''} onClick={ () => handleClick(number)}>{number}</button>)
       }
        </div>
    </div>
  )
